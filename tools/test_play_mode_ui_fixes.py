...
assert 'table.className = "ttl-skill-table ttl-skill-table-removable";' in template
assert '.ttl-skill-table-removable th:last-child' in css
assert '.ttl-skill-table-removable td:last-child' in css

play_block = template[
    template.index('<table class="ttl-skill-table ttl-play-table">'):
    template.index('</table>', template.index('<table class="ttl-skill-table ttl-play-table">')) + 8
]
assert "ttl-skill-table-removable" not in play_block
assert "<th>Skill</th><th>Description</th>" in play_block

print("PASS: v0.4.20.1 play-mode UI fixes")
