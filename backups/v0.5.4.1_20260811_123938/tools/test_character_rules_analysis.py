...
with tempfile.TemporaryDirectory() as td:
    ...
    assert "Character sheet terms potentially relevant:" in indirect
    assert "piloting" in indirect.casefold()

main=(ROOT/"app/main.py").read_text()
ask=(ROOT/"app/templates/ask.html").read_text()

assert "retrieval_query = character_retrieval_query" in main
assert "three logical steps" in main
assert "(1) identify only the sheet facts relevant" in main
assert "Do not treat absence from the character sheet as proof" in main
assert "'Character', 'Rules', and 'Conclusion'" in main
assert "Under 'Rules', cite every material rules statement." in main
assert "Character-aware rules analysis enabled" in ask

print("PASS: character-aware rules analysis")
print("  structured character evidence: OK")
print("  category-aware retrieval hints: OK")
print("  indirect-question fallback hints: OK")
