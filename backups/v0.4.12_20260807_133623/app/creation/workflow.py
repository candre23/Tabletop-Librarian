    ...
    *,
    ...
    if not isinstance(version, int) or version < 1:
        ...
        issues.append(
            CreationIssue(
                "error",
                f"Unsupported creation workflow version {version}; this TTL build supports version {CREATION_FORMAT_VERSION}.",
                "version",
            )
        )
    ...
    for index, definition in enumerate(raw_steps):
        ...
        for field_index, field_id in enumerate(fields):
            ...
            if not isinstance(field_id, str) or not field_id:
                issues.append(CreationIssue("error", "Field id must be a non-empty string.", field_location))
                continue
            if field_id not in schema.fields:
                issues.append(CreationIssue("error", f"Unknown character field {field_id!r}.", field_location))
                continue
            if field_id in seen_fields:
                issues.append(CreationIssue("error", "Field is repeated within this step.", field_location))
                continue
            if schema.fields[field_id].type == "calculated":
                issues.append(
                    CreationIssue(
                        "error",
                        "Calculated fields cannot be direct creation inputs.",
                        field_location,
                    )
                )
                continue
            ...
            clean_fields.append(field_id)
            workflow_fields.append(field_id)
    ...
    # A field may intentionally appear in only one creation step. Repeating it
    # across steps is ambiguous because step persistence/validation order matters.
    duplicate_workflow_fields = sorted(
        field_id
        for field_id in set(workflow_fields)
        if workflow_fields.count(field_id) > 1
    )
    for field_id in duplicate_workflow_fields:
        issues.append(
            CreationIssue(
                "error",
                "Character field appears in more than one creation step.",
                f"fields.{field_id}",
            )
        )
    ...
    for field_id, definition in schema.fields.items():
        ...
        if field_id not in workflow_field_set:
            issues.append(
                CreationIssue(
                    "error",
                    "Required character field has no default and is not included in the creation workflow.",
                    f"fields.{field_id}",
                )
            )
