...
async def character_creation_page(request: Request, draft_id: str):
    ...
    return _render_creation_page(
        ...
        schema=schema,
        workflow=workflow,
        unlock_core=request.query_params.get("unlock_core") == "1",
    )
...
async def character_creation_step(request: Request, draft_id: str):
    ...
    unlock_core = str(form_data.get("unlock_core") or "") == "1"
    ...
    try:
        for field_id in step.fields:
            ...
            if not _editable_field(field):
                continue
            if field_id in draft.locked_fields and not unlock_core:
                continue
    ...
    if schema_issues or reference_messages or blocking_rules or eligibility_issues:
...
async def character_save(request: Request, character_id: str):
    ...
    for field_id, field in schema.fields.items():
        if not _editable_field(field):
            continue
        if field_id in core_fields and not unlock_core:
            continue
        try:
            ...
            return _render_character_edit_page(
                ...
                status_code=400, unlock_core=unlock_core,
            )
