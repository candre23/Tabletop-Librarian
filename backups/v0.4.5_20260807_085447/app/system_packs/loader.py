...
def validate_system_pack(root: Path | str) -> SystemPack:
    ...
    if manifest.compendium:
        from app.compendium import load_compendium
        from app.characters.schema import load_character_schema

        compendium, compendium_issues = load_compendium(root, manifest.compendium)
        ...
        if compendium is not None:
            schema, _ = load_character_schema(root / manifest.character_schema)
            if schema is not None:
                for field_id, field in schema.fields.items():
                    if field.type not in {"reference", "multi_reference", "collection"}:
                        continue
                    if not field.entity:
                        continue
                    if not compendium.has_type(field.entity):
                        ...
                        continue
                    if field.type == "reference" and "default" in field.raw:
                        if field.default not in (None, "") and compendium.get(field.entity, str(field.default)) is None:
                            pack.issues.append(
                                PackIssue(
                                    "error",
                                    f"Default references unknown entity {field.entity}:{field.default}.",
                                    file=manifest.character_schema,
