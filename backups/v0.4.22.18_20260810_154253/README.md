...
### v0.4.22.17 upload staging cleanup

Incoming Upload rows now use the unified light palette with dark high-contrast
text. GMs can delete unassigned staged uploads directly from the Uploads page.
Deletion is constrained to files currently enumerated by TTL's upload staging
area. If a staged file is already registered as a physical source for a
virtual folder, deletion is blocked until that library source is removed so
the browser UI cannot silently break an existing library entry.
