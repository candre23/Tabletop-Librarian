...
def _document_record(
    ...
    if doc_type == "PDF":
        ...
        if text_status == "scanned":
            # The source PDF remains image-only by design. A persistent local
            # OCR derivative changes its Library Management status without
            # modifying the read-only source file.
