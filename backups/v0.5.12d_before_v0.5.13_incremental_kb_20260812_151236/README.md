...
- After successful OCR, TTL refreshes extracted text and context chunks automatically and starts a semantic embedding rebuild.
...
### v0.5.12d CBR/CBZ OCR support

The persistent OCR subsystem also accepts CBR and CBZ image archives. TTL assembles comic pages into a temporary local PDF, OCRs it with the existing OCRmyPDF/Tesseract workflow, and keeps the searchable PDF derivative under `data/ocr`. The source archive remains unchanged and continues to open in the native comic reader. OCR completion only marks the knowledgebase stale; the GM explicitly chooses when to run the knowledgebase update.
