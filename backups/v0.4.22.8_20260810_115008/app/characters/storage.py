...
def _apply_rules(
    ...
    try:
        ...
        return engine.apply(data, modifiers=modifiers)
...
def _validate_all(
    ...
    if compendium is not None:
        for field_id, field in schema.fields.items():
            ...
            elif field.type == "multi_reference" and isinstance(value, list):
                values = value
                ...
                for row_index, row in enumerate(value):
                    ...
                    for item_id, item_field in (field.item_schema or {}).items():
                        ...
                        if item_value in (None, ""):
                            continue
                        if compendium.get(item_field.entity, str(item_value)) is None:
                            reference_errors.append(
                                f"ERROR: {field_id}[{row_index}].{item_id}: "
                                f"Unknown {item_field.entity} reference {item_value!r}."
                            )
                continue
            else:
                continue
            ...
            for item in values:
                if compendium.get(field.entity, str(item)) is None:
                    reference_errors.append(
                        f"ERROR: {field_id}: Unknown {field.entity} reference {item!r}."
                    )

        if engine is not None:
            eligibility_errors = selected_eligibility_issues(
                schema,
                compendium,
                data,
                engine,
            )
            _limit_results, limit_issues = evaluate_limits(
                schema,
                compendium,
                data,
                engine,
    ...
    if (
        ...
        details.extend(issue.format() for issue in eligibility_errors)
        details.extend(issue.format() for issue in limit_errors)
        raise CharacterStorageError(
...
def create_character(
    ...
    *,
    ...
    data, rule_issues = _apply_rules(pack, schema, engine, compendium, data)
...
def save_character(
    ...
    *,
    ...
    _validate_all(schema, record.data, rule_issues, compendium, engine)
