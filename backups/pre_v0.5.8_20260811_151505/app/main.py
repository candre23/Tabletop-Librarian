...
from app.ai.answer_verifier import audit_answer_claims, revise_answer_from_audit
    ...
    if not error:
        ...
        try:
            ...
            if reasoning_mode == "advanced":
                update_ai_request_progress(ai_request_id, 24, "Planning searches")
                ...
                update_ai_request_progress(ai_request_id, 34, "Retrieving first-pass sources")
                planned_queries = list(plan.get("queries") or [question])[:4]
                followup_terms = list(plan.get("followup_terms") or [])[:6]
                ...
                def add_candidates(rows: list[dict]) -> None:
                    for row in rows:
                        ...
                        if not chunk_id:
                            continue
                        previous = candidates_by_id.get(chunk_id)
                        if previous is None or float(row.get("evidence_score", 0.0)) > float(
                            previous.get("evidence_score", 0.0)
                        ):
                ...
                first_pass = sorted(
                    ...
                    key=lambda row: float(row.get("evidence_score", 0.0)),
                    reverse=True,
                )
                evidence_text = "\n".join(
                    str(row.get("context_text") or row.get("text") or "")
                    for row in first_pass[:12]
                ).casefold()

                # One evidence-gated follow-up hop. Planner suggestions cannot
                # recursively expand retrieval unless the first pass actually
                # surfaced that concept.
                update_ai_request_progress(ai_request_id, 45, "Checking related rules")
                eligible_followups = [
                    term
                    for term in followup_terms
                    if str(term).strip()
                    and str(term).casefold() in evidence_text
                ][:3]

                for term in eligible_followups:
                    if cancel_event.is_set():
                        raise AIRequestCancelled("AI request cancelled.")
                    add_candidates(
                        retrieve_chunks(
            ...
            system_prompt = (
                ...
                "You may receive authoritative structured character-sheet context plus numbered source passages. "
                "Character context establishes only the selected character's current sheet state; it does not establish game rules. "
                "Numbered source passages establish rules and reference facts. "
                "Answer character-state questions from the character context without inventing missing sheet data. "
                "When a question asks how a rule applies to the selected character, determine the answer using three internal steps: "
                "(1) identify only the sheet facts materially relevant to the question, "
                "(2) identify the applicable rule text with numbered citations, and "
                "(3) derive the character-specific conclusion. "
                "Do not expose those internal steps as a mandatory report format. "
                "Lead with the direct answer in the first sentence whenever the evidence supports one. "
                "Do not assume that merely possessing a named skill, quality, item, weapon, power, or trait proves its mechanical effect; retrieve and cite the rule that defines that effect. "
                "Do not treat absence from the character sheet as proof that an action is forbidden unless a supplied rule explicitly requires the missing feature. "
                "For any rules claim, mechanical procedure, interpretation, or explanation, use only the supplied numbered source passages. "
                ...
                "When making rules/reference claims, cite the supporting numbered sources using [1], [2], etc. "
                "Do not cite character-sheet context with a numbered citation and do not pretend it came from a book. "
                "When an answer combines character facts and rules, distinguish sheet facts from sourced rules without mechanically restating the sheet. "
                "Mention only character fields that change or determine the answer. "
                "Do not list unrelated skills, gear, qualities, attributes, or other sheet contents merely to demonstrate awareness of the character. "
                "Prefer a compact response: direct answer first, followed by the minimum explanation needed and citations for the controlling rule. "
                "Use headings such as 'Character', 'Rules', and 'Conclusion' only when the question is complex enough that they improve clarity; do not use them for a simple yes/no or short rules question. "
                "If the retrieved rules are insufficient for a reliable conclusion, say what specific rule or fact is missing, without padding the answer with unrelated sheet details. "
                ...
                "A simple character-specific question should normally be answered in a few sentences, not with a full character summary."
            )
            ...
            source_text = (
                "\n\n".join(source_blocks)
                if source_blocks
                else "(No relevant numbered source passages were retrieved.)"
            )
            reasoning_note = (
                "Retrieval mode: Advanced semantic planning with bounded multi-query and one-hop follow-up retrieval."
                if reasoning_mode == "advanced"
                else "Retrieval mode: Basic single-query retrieval."
            )
            user_prompt = (
                f"Question:\n{question}\n\n"
                f"{reasoning_note}\n\n"
            ...
            update_ai_request_progress(
                ai_request_id,
                60 if reasoning_mode == "advanced" else 58,
                "Generating draft answer" if reasoning_mode == "advanced" else "Generating answer",
            )
            ...
            if reasoning_mode == "advanced":
                update_ai_request_progress(ai_request_id, 72, "Draft answer received")
                update_ai_request_progress(ai_request_id, 77, "Auditing mechanical claims")
                ...
                if findings:
                    update_ai_request_progress(
                        ai_request_id,
                        87,
                        f"Revising {len(findings)} flagged claim"
                        + ("" if len(findings) == 1 else "s"),
                    )
                    revised = await asyncio.to_thread(
                        revise_answer_from_audit,
                        question=question,
                        draft_answer=draft_answer,
                        audit=audit,
                        character_block=character_block,
                    ...
                    answer = revised["content"] or draft_answer
                    answer_model = revised.get("model") or answer_model
                    update_ai_request_progress(ai_request_id, 94, "Revised answer received")
                else:
                    answer = draft_answer
                    update_ai_request_progress(ai_request_id, 94, "Audit passed")
            else:
                answer = draft_answer
