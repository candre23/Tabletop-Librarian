    ...
    if not error:
        ...
        try:
            ...
            if reasoning_mode == "advanced":
                ...
                retrieval_batches: list[tuple[str, list[dict]]] = []
                ...
                for planned_query in planned_queries:
                    ...
                    retrieval_batches.append((planned_query, rows))

                first_pass_rows = [
                    row
                    for _, rows in retrieval_batches
                    for row in rows
                ]
                evidence_text = "\n".join(
                ...
                for term in eligible_followups:
                    if cancel_event.is_set():
                        raise AIRequestCancelled("AI request cancelled.")
                    followup_query = f"{question} {term}"
                    rows = retrieve_chunks(
                        followup_query,
                        user["role"],
                        limit=4,
                    ...
                    retrieval_batches.append((followup_query, rows))

                update_ai_request_progress(ai_request_id, 52, "Ranking evidence")
            ...
            update_ai_request_progress(
                ai_request_id,
                58 if reasoning_mode == "advanced" else 52,
                "Assembling evidence",
            )
            source_blocks = []
            ...
            reasoning_note = (
                "Retrieval mode: Advanced planning with bounded multi-query retrieval and deterministic evidence ranking."
                if reasoning_mode == "advanced"
            ...
            update_ai_request_progress(
                ai_request_id,
                68 if reasoning_mode == "advanced" else 58,
                "Generating final answer" if reasoning_mode == "advanced" else "Generating answer",
            )
            ...
            if reasoning_mode == "advanced":
                update_ai_request_progress(ai_request_id, 92, "Final answer received")
