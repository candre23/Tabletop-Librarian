...
def _reader_thread(stream: Any, total_pages: int, output_chunks: list[str]) -> None:
    try:
        tail = ""
        while True:
            ...
            # Bound retained diagnostics to roughly the last 64 KiB.
            while sum(len(part) for part in output_chunks) > 65536 and len(output_chunks) > 1:
                output_chunks.pop(0)
            combined = tail + chunk
...
def _run_ocr(source: Path) -> Path:
    ...
    reader = Thread(target=_reader_thread, args=(process.stdout, total_pages, output_chunks), daemon=True)
    reader.start()
