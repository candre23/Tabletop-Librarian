...
Normal **Update Knowledgebase** runs are incremental. TTL scans source signatures, extracts only added or changed documents, removes deleted documents, rebuilds chunks only for affected paths, and reuses semantic vectors for every unchanged chunk. **Rebuild Entire Knowledgebase** remains available as an explicit maintenance action after changing embedding models/indexing behavior or when a clean rebuild is desired.
...
### v0.5.11 GM-managed pipeline selection

- Moves the Advanced Ask pipeline preset selector from the Ask pane to GM-only Knowledgebase Tools → Local AI Provider.
- Stores the selected preset with the local AI provider settings and applies it automatically to all Advanced Ask requests.
- Players cannot choose or submit pipeline presets from Ask.

### v0.5.12 persistent local OCR workflow

- Adds GM-managed OCR for PDFs detected as scanned/image-only documents.
- Uses OCRmyPDF with Tesseract, page rotation, deskewing, mixed-page text skipping, and one OCR worker at a time.
- Source PDFs are never modified. OCR derivatives are stored persistently under `data/ocr/pdf/`, with source signatures under `data/ocr/meta/` so read-only network libraries are supported.
- A stored OCR derivative remains valid across restarts until the original source file changes; changed sources automatically require OCR again.
- Knowledgebase text extraction transparently uses the local OCR derivative while retaining the original library path as the document identity and citation source.
- Knowledgebase Tools can OCR one required PDF or all required PDFs, reports background-job progress, and supports cancellation.
- After successful OCR, TTL marks the knowledgebase as needing an update but does not start CPU-intensive indexing automatically. The next normal Update Knowledgebase processes only the OCR-completed document(s).
- OCR is an optional host dependency. On Ubuntu/Debian, install OCRmyPDF with `sudo apt install ocrmypdf` (the package brings its required OCR/runtime dependencies).


### v0.5.13a knowledgebase notice hotfix

- Knowledgebase update warnings now clear automatically when the background embedding stage finishes.
- One-time success notices are removed from the browser URL after display, so they do not persist across refreshes or TTL restarts.

### v0.5.13 incremental knowledgebase updates

- Makes **Update Knowledgebase** incremental by default. Added, modified, newly OCR-completed, and removed documents are detected from the extracted-text cache/source signatures.
- Rebuilds context chunks only for affected documents and preserves chunks for unchanged books.
- Reuses existing semantic vectors for unchanged chunks and generates embeddings only for new or changed chunks.
...
- A source that is temporarily missing or inaccessible is treated as offline rather than deleted. Its virtual-library entries, extracted text, chunks, semantic vectors, and OCR derivatives are preserved.
- Absence is treated as deletion only after TTL successfully scans the relevant source, or when a reachable configured ancestor proves that a previously registered descendant source was actually removed. Explicitly removing a source/folder remains an intentional deletion.
