...
def _system_vocabulary(
    schema,
    compendium: Compendium,
    *,
    limit: int = 280,
) -> list[str]:
    """Bounded System Pack terminology for semantic retrieval planning."""
    values: list[str] = []

    for field_id, field in schema.fields.items():
        values.append(field.label)
        values.append(field_id.replace("_", " "))
        if field.entity:
            values.append(str(field.entity).replace("_", " "))
        for item_id, item_field in (field.item_schema or {}).items():
            values.append(item_field.label)
            values.append(item_id.replace("_", " "))
            if item_field.entity:
                values.append(str(item_field.entity).replace("_", " "))

    for entity_type, bucket in compendium.entities.items():
        values.append(str(entity_type).replace("_", " "))
        for entity in bucket.values():
            values.append(entity.name)
            values.extend(entity.tags)

    return _dedupe_hints(values, limit=limit)
...
def build_character_ai_context(
    ...
    *,
    ...
    return {
        ...
        "system_vocabulary": _system_vocabulary(schema, compendium),
        "text": "\n".join(lines).strip(),
    }
